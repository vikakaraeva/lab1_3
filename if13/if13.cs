    ...
    class if13
    {
        static void Main(string[] args)
        {
            ...
            while (noError)// цикл while для try-catch: если ввели с клавиатуры НЕ число, то возвращаемся обратно
            {
                ...
                if ((ch1 >= ch2)&(ch1<=ch3))//условие
                {
                sred = ch1;
                }
                if ((ch1 >= ch3) & (ch1<=ch2))//условие
                {
                sred = ch1;
                }
                if ((ch2 >= ch1) & (ch2 <= ch3))//условие
                {
                    sred = ch2;
                }
                if ((ch2 <= ch1) & (ch2 >= ch3))//условие
                ...
                if ((ch3 >= ch2) & (ch3 <= ch1))//условие
                {
                    sred = ch3;
                }
                if ((ch3 >= ch1) & (ch3 <= ch2))//условие
                {
                    sred = ch3;
                }

            Console.WriteLine("cреднее число: "+sred); //вывод результата
